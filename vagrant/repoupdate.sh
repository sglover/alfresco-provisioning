...
SYNC_SERVER_HOSTNAME=`java -jar service-sync-bundle-1.1-SNAPSHOT.jar -hostname $1 $2 $4`
if [[ $? == "0" ]]; then
    ...
    java -jar service-sync-bundle-1.1-SNAPSHOT.jar -syncuri $JMX_USER $JMX_PASS $SYNC_SERVER_URI  
else
    echo "Sync server instance is not running"
fi

echo "Getting services hostname"
SERVICES_SERVER_HOSTNAME=`java -jar service-sync-bundle-1.1-SNAPSHOT.jar -hostname $1 $2 $3`
if [[ $? == "0" ]]; then
    ...
    java -jar service-sync-bundle-1.1-SNAPSHOT.jar -brokeruri $JMX_USER $JMX_PASS $SERVICES_SERVER_HOSTNAME
else
