...
public class BMService
{
     ...
     *             if unsupported
     */
    public StringEntity setMessageBody(final String content)
            throws UnsupportedEncodingException
    {
    ...
    public JSONObject readStream(final HttpEntity entity)
    {
        ...
        {
            throw new RuntimeException("Failed to read HTTP entity stream.",
                    ex);
        }
        finally
        {
     ...
     * 
     * @param method
     *            the method containing the response
     * @return the "data" object. Returns null if response is not JSON or no
    ...
    public JSONObject getTestProperty(String testName, String propertyName)
            throws IOException
    {
        ...
        try
        {
            String url = "http://" + hostname
                    + ":9080/alfresco-benchmark-server/api/v1/tests/" + testName
                    + "/props/" + propertyName;
            ...
            {
                System.err.println("Property " + propertyName + " for test "
                        + testName + " does not exist");
            }
            else
            {
                System.err.println(
                        "Unexpected response " + httpResponse.toString());
            }
    ...
    public void setTestProperty(String testName, String propertyName,
            Object propertyValue) throws IOException
    {
        ...
        if (json != null)
        {
            ...
            {
                String url = "http://" + hostname
                        + ":9080/alfresco-benchmark-server/api/v1/tests/"
                        + testName + "/props/" + propertyName;
    ...
    public void runTest(String testName, String testRunName, String testRunDescription) throws IOException
    {
        ...
        try
        {
            ...
            if (status.getStatusCode() == HttpStatus.SC_OK)
            {
                url = "http://" + hostname
                        + ":9080/alfresco-benchmark-server/api/v1/tests/"
                        + testName + "/runs/" + testRunName + "/schedule";
